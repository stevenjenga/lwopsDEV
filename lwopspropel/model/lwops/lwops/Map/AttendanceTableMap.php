...
use lwops\lwops\Attendance;
use lwops\lwops\AttendanceQuery;
...
class AttendanceTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'lwops.lwops.Map.AttendanceTableMap';
     ...
     */
    const OM_CLASS = '\\lwops\\lwops\\Attendance';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'lwops.lwops.Attendance';
     ...
     */
    const COL_OID = 'attendance.oid';

    /**
     * the column name for the employeeOid field
     */
    const COL_EMPLOYEEOID = 'attendance.employeeOid';

    /**
     * the column name for the attendanceDt field
     */
    const COL_ATTENDANCEDT = 'attendance.attendanceDt';

    /**
     * the column name for the attendance_in field
     */
    const COL_ATTENDANCE_IN = 'attendance.attendance_in';

    /**
     * the column name for the createTmstp field
     */
    const COL_CREATETMSTP = 'attendance.createTmstp';

    /**
     * the column name for the updtTmstp field
     */
    const COL_UPDTTMSTP = 'attendance.updtTmstp';
    ...
    protected static $fieldNames = array (
        self::TYPE_PHPNAME       => array('Oid', 'Employeeoid', 'Attendancedt', 'AttendanceIn', 'Createtmstp', 'Updttmstp', ),
        self::TYPE_CAMELNAME     => array('oid', 'employeeoid', 'attendancedt', 'attendanceIn', 'createtmstp', 'updttmstp', ),
        self::TYPE_COLNAME       => array(AttendanceTableMap::COL_OID, AttendanceTableMap::COL_EMPLOYEEOID, AttendanceTableMap::COL_ATTENDANCEDT, AttendanceTableMap::COL_ATTENDANCE_IN, AttendanceTableMap::COL_CREATETMSTP, AttendanceTableMap::COL_UPDTTMSTP, ),
        self::TYPE_FIELDNAME     => array('oid', 'employeeOid', 'attendanceDt', 'attendance_in', 'createTmstp', 'updtTmstp', ),
    ...
    protected static $fieldKeys = array (
        self::TYPE_PHPNAME       => array('Oid' => 0, 'Employeeoid' => 1, 'Attendancedt' => 2, 'AttendanceIn' => 3, 'Createtmstp' => 4, 'Updttmstp' => 5, ),
        self::TYPE_CAMELNAME     => array('oid' => 0, 'employeeoid' => 1, 'attendancedt' => 2, 'attendanceIn' => 3, 'createtmstp' => 4, 'updttmstp' => 5, ),
        self::TYPE_COLNAME       => array(AttendanceTableMap::COL_OID => 0, AttendanceTableMap::COL_EMPLOYEEOID => 1, AttendanceTableMap::COL_ATTENDANCEDT => 2, AttendanceTableMap::COL_ATTENDANCE_IN => 3, AttendanceTableMap::COL_CREATETMSTP => 4, AttendanceTableMap::COL_UPDTTMSTP => 5, ),
        self::TYPE_FIELDNAME     => array('oid' => 0, 'employeeOid' => 1, 'attendanceDt' => 2, 'attendance_in' => 3, 'createTmstp' => 4, 'updtTmstp' => 5, ),
        self::TYPE_NUM           => array(0, 1, 2, 3, 4, 5, )
    );
    ...
    public function initialize()
    {
        ...
        $this->setClassName('\\lwops\\lwops\\Attendance');
        ...
        $this->addColumn('attendanceDt', 'Attendancedt', 'TIMESTAMP', true, null, null);
        $this->addColumn('attendance_in', 'AttendanceIn', 'INTEGER', true, null, 0);
  ...
  array (
    ...
    1 => ':oid',
        ...
        $this->addRelation('Teapicking', '\\lwops\\lwops\\Teapicking', RelationMap::ONE_TO_MANY, array (
  0 =>
  array (
    0 => ':attendanceOid',
    1 => ':oid',
  ),
), null, null, 'Teapickings', false);
        $this->addRelation('Teapruning', '\\lwops\\lwops\\Teapruning', RelationMap::ONE_TO_MANY, array (
  0 =>
  array (
    0 => ':attendanceOid',
    1 => ':oid',
  ),
), null, null, 'Teaprunings', false);
    } // buildRelations()
    ...
    {
        if (null === $alias) {
            $criteria->addSelectColumn(AttendanceTableMap::COL_OID);
            $criteria->addSelectColumn(AttendanceTableMap::COL_EMPLOYEEOID);
            $criteria->addSelectColumn(AttendanceTableMap::COL_ATTENDANCEDT);
            $criteria->addSelectColumn(AttendanceTableMap::COL_ATTENDANCE_IN);
            $criteria->addSelectColumn(AttendanceTableMap::COL_CREATETMSTP);
            $criteria->addSelectColumn(AttendanceTableMap::COL_UPDTTMSTP);
        } else {
            $criteria->addSelectColumn($alias . '.oid');
            $criteria->addSelectColumn($alias . '.employeeOid');
            $criteria->addSelectColumn($alias . '.attendanceDt');
            $criteria->addSelectColumn($alias . '.attendance_in');
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \lwops\lwops\Attendance) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(AttendanceTableMap::DATABASE_NAME);
            $criteria->add(AttendanceTableMap::COL_OID, (array) $values, Criteria::IN);
        }
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria->containsKey(AttendanceTableMap::COL_OID) && $criteria->keyContainsValue(AttendanceTableMap::COL_OID) ) {
