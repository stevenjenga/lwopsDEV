...
/**
 * Base class that represents a row from the 'horticulturesellunit' table.
 *
 *
 *
 * @package    propel.generator.lwops.lwops.Base
 */
abstract class Horticulturesellunit implements ActiveRecordInterface
{
    /**
     * TableMap class name
     */
    const TABLE_MAP = '\\lwops\\lwops\\Map\\HorticulturesellunitTableMap';
     ...
     *
     * Note: this column has a database default value of: 'Kilogram'
     * @var        string
     */
    ...
    {
        $this->unit = 'KG';
        $this->description = 'Kilogram';
    }

    /**
     * Initializes internal state of lwops\lwops\Base\Horticulturesellunit object.
     * @see applyDefaults()
     */
    ...
    /**
     * Compares this with another <code>Horticulturesellunit</code> instance.  If
     * <code>obj</code> is an instance of <code>Horticulturesellunit</code>, delegates to
     * <code>equals(Horticulturesellunit)</code>.  Otherwise, returns <code>false</code>.
     *
     ...
     *
     * @return $this|Horticulturesellunit The current object, for fluid interface
     */
     ...
     * @return $this|\lwops\lwops\Horticulturesellunit The current object (for fluent API support)
     */
    public function setUnit($v)
    {
        ...
        if ($this->unit !== $v) {
            $this->unit = $v;
            $this->modifiedColumns[HorticulturesellunitTableMap::COL_UNIT] = true;
        }
    ...
    public function setDescription($v)
    {
        ...
        if ($this->description !== $v) {
            $this->description = $v;
            $this->modifiedColumns[HorticulturesellunitTableMap::COL_DESCRIPTION] = true;
        }
    ...
    public function setCreatetmstp($v)
    {
        ...
        if ($this->createtmstp !== null || $dt !== null) {
            if ($this->createtmstp === null || $dt === null || $dt->format("Y-m-d H:i:s.u") !== $this->createtmstp->format("Y-m-d H:i:s.u")) {
                ...
                $this->modifiedColumns[HorticulturesellunitTableMap::COL_CREATETMSTP] = true;
            }
    ...
    public function setUpdttmstp($v)
    {
        ...
        if ($this->updttmstp !== null || $dt !== null) {
            if ($this->updttmstp === null || $dt === null || $dt->format("Y-m-d H:i:s.u") !== $this->updttmstp->format("Y-m-d H:i:s.u")) {
                ...
                $this->modifiedColumns[HorticulturesellunitTableMap::COL_UPDTTMSTP] = true;
            }
        } // if either are not null
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : HorticulturesellunitTableMap::translateFieldName('Unit', TableMap::TYPE_PHPNAME, $indexType)];
            $this->unit = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : HorticulturesellunitTableMap::translateFieldName('Description', TableMap::TYPE_PHPNAME, $indexType)];
            $this->description = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : HorticulturesellunitTableMap::translateFieldName('Createtmstp', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            $this->createtmstp = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : HorticulturesellunitTableMap::translateFieldName('Updttmstp', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            ...
            return $startcol + 4; // 4 = HorticulturesellunitTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\lwops\\lwops\\Horticulturesellunit'), 0, $e);
        }
    }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildHorticulturesellunitQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
     ...
     * @return void
     * @throws PropelException
     * @see Horticulturesellunit::setDeleted()
     * @see Horticulturesellunit::isDeleted()
     */
    public function delete(ConnectionInterface $con = null)
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildHorticulturesellunitQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                HorticulturesellunitTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(HorticulturesellunitTableMap::COL_UNIT)) {
            $modifiedColumns[':p' . $index++]  = 'unit';
        }
        if ($this->isColumnModified(HorticulturesellunitTableMap::COL_DESCRIPTION)) {
            $modifiedColumns[':p' . $index++]  = 'description';
        }
        if ($this->isColumnModified(HorticulturesellunitTableMap::COL_CREATETMSTP)) {
            $modifiedColumns[':p' . $index++]  = 'createTmstp';
        }
        if ($this->isColumnModified(HorticulturesellunitTableMap::COL_UPDTTMSTP)) {
            $modifiedColumns[':p' . $index++]  = 'updtTmstp';
        }

        $sql = sprintf(
            'INSERT INTO horticulturesellunit (%s) VALUES (%s)',
    ...
    {
        $pos = HorticulturesellunitTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if (isset($alreadyDumpedObjects['Horticulturesellunit'][$this->hashCode()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['Horticulturesellunit'][$this->hashCode()] = true;
        $keys = HorticulturesellunitTableMap::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getUnit(),
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\lwops\lwops\Horticulturesellunit
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\lwops\lwops\Horticulturesellunit The current object, for fluid interface
     */
    ...
    {
        $criteria = new Criteria(HorticulturesellunitTableMap::DATABASE_NAME);

        if ($this->isColumnModified(HorticulturesellunitTableMap::COL_UNIT)) {
            $criteria->add(HorticulturesellunitTableMap::COL_UNIT, $this->unit);
        }
        if ($this->isColumnModified(HorticulturesellunitTableMap::COL_DESCRIPTION)) {
            $criteria->add(HorticulturesellunitTableMap::COL_DESCRIPTION, $this->description);
        }
        if ($this->isColumnModified(HorticulturesellunitTableMap::COL_CREATETMSTP)) {
            $criteria->add(HorticulturesellunitTableMap::COL_CREATETMSTP, $this->createtmstp);
        }
        if ($this->isColumnModified(HorticulturesellunitTableMap::COL_UPDTTMSTP)) {
            $criteria->add(HorticulturesellunitTableMap::COL_UPDTTMSTP, $this->updttmstp);
        }
    ...
    {
        $criteria = ChildHorticulturesellunitQuery::create();
        $criteria->add(HorticulturesellunitTableMap::COL_UNIT, $this->unit);
    ...
    /**
     * Generic method to set the primary key (unit column).
     *
     ...
     * objects.
     *
     * @param      object $copyObj An object of \lwops\lwops\Horticulturesellunit (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \lwops\lwops\Horticulturesellunit Clone of current object.
     * @throws PropelException
     */
     ...
     * If this ChildHorticulturesellunit is new, it will return
        ...
        if (null === $this->collHorticulturesaless || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collHorticulturesaless) {
                ...
                $collHorticulturesaless = ChildHorticulturesalesQuery::create(null, $criteria)
                    ->filterByHorticulturesellunit($this)
                    ->find($con);
     ...
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildHorticulturesellunit The current object (for fluent API support)
     */
    public function setHorticulturesaless(Collection $horticulturesaless, ConnectionInterface $con = null)
    {
        ...
        foreach ($horticulturesalessToDelete as $horticulturesalesRemoved) {
            $horticulturesalesRemoved->setHorticulturesellunit(null);
        }
    ...
    protected function doAddHorticulturesales(ChildHorticulturesales $horticulturesales)
    {
        ...
        $horticulturesales->setHorticulturesellunit($this);
    }
    ...
    public function removeHorticulturesales(ChildHorticulturesales $horticulturesales)
    {
        if ($this->getHorticulturesaless()->contains($horticulturesales)) {
            ...
            $horticulturesales->setHorticulturesellunit(null);
        }
     ...
     * Otherwise if this Horticulturesellunit is new, it will return
     * an empty collection; or if this Horticulturesellunit has previously
     * been saved, it will retrieve related Horticulturesaless from storage.
     *
     * This method is protected by default in order to keep the public
     * api reasonable.  You can provide public methods for those you
     * actually need in Horticulturesellunit.
     *
     * @param      Criteria $criteria optional Criteria object to narrow the query
     * @param      ConnectionInterface $con optional connection object
    ...
    {
        return (string) $this->exportTo(HorticulturesellunitTableMap::DEFAULT_STRING_FORMAT);
    }
