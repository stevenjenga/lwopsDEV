...
function loadCowNamesCombo(){
	global $db, $logger;
	$xml = '<?xml version="1.0" encoding="UTF-8"?>'.
		'<complete>';
	
	$rows = $db->query("SELECT oid, UPPER(name) AS name FROM dairycowname ORDER BY UPPER(name) ");
    $logger->debug('loadCowNamesCombo()', $db->getLastQuery());
	foreach ($rows as $row) {
		$xml .= '<option value="'.$row['oid'].'"><![CDATA['.$row['name'].']]></option>';
	}
	$xml .= '</complete>';
