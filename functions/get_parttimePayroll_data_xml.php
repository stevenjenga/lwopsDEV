...
try {
	...
	if($rows){
		...
		foreach($rows as $row){
			...
			$sql2 = "CREATE TABLE tempPartTimeTable AS SELECT eOid, CONCAT(firstName, ' ', middleinitial, ' ', lastName) AS employeeNm,". $row['aRow']." FROM empPartTimeHrs_vw GROUP BY eOid"; 
			$db->trace = array();
			$rows = $db->query($sql2);
			$logger->debug(pathinfo(__FILE__, PATHINFO_FILENAME)."[tempPartTimeTable Qry]", $db->trace);
			
			//get payroll data from temporary pivot table
			$db->trace = array();
			$rows = $db->query("SELECT * FROM tempPartTimeTable ORDER BY employeeNm");
			$logger->debug(pathinfo(__FILE__, PATHINFO_FILENAME), $db->trace);
			...
			print('<column width="50" type="ro" align="middle" sort="str">Type</column>');
			print('<column width="100" type="kenyaCurrencyro" align="right" sort="str">Base Pay</column>');
			...
			$sql = "DROP TABLE tempPartTimeTable";
			$db->trace = array();
			$rows = $db->query($sql);
			$logger->debug(pathinfo(__FILE__, PATHINFO_FILENAME), $db->trace);
		}
...
function getColumnTotals($columnNames){
	...
	for ($i=2; $i<16; $i++) {
		$columnTotalSql = "SELECT SUM(".$columnNames[$i].") AS dayTotal FROM `tempPartTimeTable` ";
		$db->trace = array();
