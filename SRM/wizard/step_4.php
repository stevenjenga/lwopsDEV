<?php
	...
	if(isset($_SESSION["srm_f62014_datasource"])) $datasource =  $_SESSION["srm_f62014_datasource"];
	else header("location: $url?id=0");
	
	$allFields = array();
	// set data from session to fields
	if($datasource === "table")
	{
		if(isset($_SESSION["srm_f62014_table"])) $table = $_SESSION["srm_f62014_table"];
		else header("location: $url?id=1");
		...
		foreach($table as $key => $val)
		{
			...
			{   
				if (count($table) === 1 ) $allFields[] = $row[0];
				else $allFields[] = $val.".".$row[0];
			}
		}		
	}else{

		if(isset($_SESSION["srm_f62014_sql"])) $sql = $_SESSION["srm_f62014_sql"];
		else header("location: $url?id=1");
		...
		if($rows !== 0) foreach($result[0] as $key => $val) $allFields[]= $key;
		else $error .= "<br>*Records = 0, please click back and enter another query" ;
    ...
    if(empty($labels) || count($labels) < 1)
    {
        ...
        foreach ($selectedFields as $field)
        {
            if(array_key_exists($field, $labels)) $temp[$field] =  $labels[$field];
            else
            {
                $chunks = explode('.', $field);
						...
						<div class="row">
								...
								<div class="left-inner-addon">
									...
									<?php
										foreach($allFields as $val)
										{
											...
											{
												if (!in_array($val, $_SESSION["srm_f62014_fields"])) echo "<option value='$val'>$val</option>";
							...
							<div class="form-group col-xs-5" style="margin: 0px; padding: 0px 0px 0px 5px;">
								...
								<div class="left-inner-addon">
									...
									<?php							
										if(isset($_SESSION["srm_f62014_fields"]))
										foreach($_SESSION["srm_f62014_fields"] as $val) if(in_array($val, $allFields)) echo "<option value='$val'>$val</option>";																
									?>
									</select>
								</div>
							</div>
							...
							<div class="col-xs-10 table-container">
								<table id="labels-table" class="table table-hover">
									<?php foreach($labels as $key => $val) {?>
										<tr>
											<td style="width: 50%;"><label for="lbl_<?php echo str_replace(array('.', ' '), array('0x', 'x0Space'), $key); ?>"><?php echo $key; ?></label></td>
											<td style="width: 50%;">
												<div class="left-inner-addon">
													<i class="glyphicon glyphicon-edit"></i>
													<input class="input-as-tf" type="text" value="<?php echo $val; ?>" id="lbl_<?php echo str_replace(array('.', ' '), array('0x', 'x0Space'), $key); ?>" name="lbl_<?php echo str_replace(array('.', ' '), array('0x', 'x0Space'), $key); ?>" />
												</div>
											</td>
										</tr>
									<?php } ?>
								...
								<div class="left-inner-addon">
									...
									<select name="functions" class="form-control font-size-lg" id="functions">
										<?php
											...
											{
												if(isset($_SESSION["srm_f62014_function"]) && $_SESSION["srm_f62014_function"] === $func ) echo "<option value='$func' selected>$func</option>";
												else echo "<option value='$func'>$func</option>";
											}
										?>
									</select>
								</div>
						...
						<div class="row" style="position: relative;top: -10px;">
							...
							<div class="col-xs-3">
								<button name="set-statistical" class="btn btn-primary btn-block btn-xs" style="font-size: 12px;" id="set-statistical">
									<?php 
										if(isset($_SESSION["srm_f62014_statestical"]) && $_SESSION["srm_f62014_statestical"] === 1) echo 'Unset Function'; 
										else echo 'Set Function'; 
									?>
								</button/> 
							</div>
							<div class="col-xs-1"></div>
...
<script language ="javascript">
	...
	var s_selectedFields = <?php if(isset($_SESSION["srm_f62014_fields"])) echo 'new Array("' . implode('", "', $_SESSION["srm_f62014_fields"]) . '")';
							else echo "''";?>;
	...
	$(function(){
		...
		$("#page-header").empty();
		$("#page-header").append('<div id="img-container"><img src="designLayer/images/columns.jpg" width="70" height="70"/></div>');
		...
		$("#statistical-nav").click(function(e){
			...
			{
				$("#page-header").empty();
				$("#page-header").append('<div id="img-container"><img src="designLayer/images/Statistics.jpg" width="70" height="70"/></div>');
		...
		$("#labels-nav").click(function(e){
			...
			{
				$("#page-header").empty();
				$("#page-header").append('<div id="img-container"><img src="designLayer/images/labels.jpg" width="70" height="70"/></div>');
		...
		$("#select-columns").mousedown(function(){
			...
			$.ajax({
				url: "server/step_4.php",
				type: "post",
				data: "selectedFields="+selectedFieldsAjax,
				success: function(data){
					...
					if(data === "success") 
					{
						...
						{
							if(selectedFields.length === 1) $("#groupby_column").append("<option value='None'>None</option>");
							...
							for(var i = 0; i < selectedFields.length; i++)
							{
								...
								var key = selectedFields[i].replace('.', '0x');
								var chunks = selectedFields[i].split('.');
								var val = (selectedFields[i].indexOf('.') === -1) ? selectedFields[i] : chunks[1];
								$("#labels-table").append("<tr><td style='width: 50%;'><label for='lbl_"+key+"'>"+selectedFields[i]+"</label></td>"+
									"<td style='width: 50%;'><div class='left-inner-addon'><i class='glyphicon glyphicon-edit'></i>"+
									"<input class='input-as-tf' type='text' value='"+val+"' id='lbl_"+key+"' name='lbl_"+key+"' /></div></td></tr>"
								);								
							}
						}
			...
			$.ajax({
				...
				success: function(data){
					...
					}else if(data === "unset_success")
					{
						...
						if(data === "error1") $("#statistical-error-container").append("<div class='alert alert-danger'>* Please select a function</div>");
						else if(data === "error2") $("#statistical-error-container").append("<div class='alert alert-danger'>* Please select an affected column</div>");
						else if(data === "error3") $("#statistical-error-container").append("<div class='alert alert-danger'>* Please select a group by column</div>");
						else if(data === "error4") $("#statistical-error-container").append("<div class='alert alert-danger'>* Affected column and Group by can't be the same, Please pick another Affected column or Group by</div>");
