...
if (is_exist($_GET['export'])) {
    if(in_array($_GET['export'], $_export_option_options)){    $_export_option = $_GET['export'];
    }
    else{
    $_export_option = "";
    $_GET['export'] = "";
...
if ($possible_attack === true) {
    if (check_debug_mode() == 1) {
      ...
      die('<div class="nav" style="padding: 8px;text-align: center;color: white;">Special Characters are not allowed for security reasons!</div>');
    exit;
}
...
if (count($result) < 1 || empty($result)) {
    if (check_debug_mode() == 1) {
      ...
      die('<div class="nav" style="padding: 8px;text-align: center;color: white;">No Data Found</div>');
}
