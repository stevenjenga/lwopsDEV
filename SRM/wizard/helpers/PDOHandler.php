		...
		public function query($sqlStatement, $keyType = "NUM", $params = array()) // $keyType = ASSOC, NUM, BOTH
		{
			...
			if ($this->link)
			{
				try{
					...
					if($query && count($params) > 0) $result = @$query->execute($params);
					...
					if(!$result)
					{
						if ($this->debug)
						{
							...
							$str = ob_get_clean();
							$this->debug_mode('query', 'error', '#Query Failed<br/>'.$str.
								'<br />check array of parameters it must be like that array(param1, param2, ...);');
						}
						return false;
					}else{
				...
				try{
					...
					if(!$result)
					{
						if ($this->debug)
						{
							...
							var_dump($query->errorInfo());
							$str = ob_get_clean();
							$this->debug_mode('command', 'error', '#Command Failed<br/>'.$str.
								'<br />check array of parameters it must be like that array(param1, param2, ...);');
						}
						return false;
					}else{
