...
//use Srequire_once('../functions/functions.php');
...
//        $logger->debug(pathinfo(__FILE__, PATHINFO_FILENAME), ['MYSQLDB _stmtError: ' => $stmt->error]);
//        $logger->debug(pathinfo(__FILE__, PATHINFO_FILENAME), ['MYSQLDB _stmtErrno: ' => $stmt->errno]);
//        $logger->debug(pathinfo(__FILE__, PATHINFO_FILENAME), ['MYSQLDB $haveOnDuplicate: ' => $haveOnDuplicate]);
//        $logger->debug(pathinfo(__FILE__, PATHINFO_FILENAME), ['MYSQLDB affected_rows: ' => $stmt->affected_rows]);
...
//        $logger->debug(pathinfo(__FILE__, PATHINFO_FILENAME), ['_buildQuery()' => $stmt]);
//        $logger->debug(pathinfo(__FILE__, PATHINFO_FILENAME), ['_buildQuery()' => $this->_bindParams]);
    ...
    {
        global $db,$logger;
//        $logger->debug(pathinfo(__FILE__, PATHINFO_FILENAME), ['_prepareQuery()' => $this->_query]);
    ...
    {
		$qry = array();
        return $qry[]= array($this->_lastQuery);
    }
