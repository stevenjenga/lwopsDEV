...
if(!empty($back))
{
    if($_SESSION['sfm_f314_data_source'] == 'sql')
 ...
 if(!empty($continue))
 {
     ...
     {
         $_SESSION["sfm_f314_fields"] = array();
        $error .= "* At least One field should be selected" ;
      }
     else
     {
         $_SESSION["sfm_f314_fields"]= clean_array($SelectedFields);
         //echo print_r($_SESSION["sfm_f314_fields"]);
         header("location: step_4.php");
     }
 }
 $table = $_SESSION['sfm_f314_table'];
 $fields = isset($_SESSION["sfm_f314_fields"]) ? $_SESSION["sfm_f314_fields"] : array();
 $insert = substr($_SESSION['sfm_f314_permission'], 0, 1) == '1'?true:false;
  
 if(empty ($table)) header("location: step_2.php");
 //$_SESSION["sfm_f314_fields"]
 $query = 'SHOW COLUMNS FROM `'.$table.'`';
 $result = $dbHandler->query($query, 'ASSOC');
 $DD_Selected = '';
 $DD_Avaliable = '';
	...
	<tr>
		<td align="center" width="64" style="background-repeat: y" valign="top" background="images/leftadd.jpg">
			...
			<table border="0" width="100%" id="table8" height="331">
				<tr>
					<td height="18" colspan="2" class="step_title">Select Fields
					
										  <button id="exit" style="position: relative;left: 340px;font-size: 11px;cursor: pointer;" onclick="return false;">Disconnect &amp; Exit</button></td>
				</tr>
				<tr>
