...
$table = $_SESSION['sfm_f314_details_table'];
$column = $_SESSION['sfm_f314_details_column'];
...
@$result = $dbHandler->query('SHOW COLUMNS FROM `'.$table.'` WHERE Field not IN(?)', 'ASSOC', array($column), 's');// WHERE Field IN('.$SQL_IN_FORMAT_FIELDS.')');
...
if($autodetect)
{
	...
	$sql = 'select CONSTRAINT_NAME,TABLE_NAME,COLUMN_NAME,REFERENCED_TABLE_NAME,REFERENCED_COLUMN_NAME 
		from information_schema.key_column_usage where referenced_table_name is not null and COLUMN_NAME IN('.$place_holders.') and TABLE_NAME = ?';
	...
	<tr>
		<td align="center" width="64" style="background-repeat: y" valign="top" background="images/leftadd.jpg">
			...
			<table border="0" width="100%" id="table8" height="331">
				<tr>
					<td height="18" colspan="2" class="step_title">Details form fields <a href="" onMouseOver="stm(Step_4[0],Style);" onClick="return false;" onMouseOut="htm()"> <img src="images/Help.png" border="0"></a>
					<button id="exit" style="position: relative;left: 330px;font-size: 11px;cursor: pointer;z-index: 0;" onclick="return false;">Disconnect &amp; Exit</button></td>
				</tr>
				<tr>
