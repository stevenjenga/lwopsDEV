...
$table = $_SESSION['sfm_f314_table'];
$fields = $_SESSION["sfm_f314_fields"];
$autodetect = $_SESSION['sfm_f314_autodetect'];
$SQL_IN_FORMAT_FIELDS = '"'.str_replace(',', '","', implode($fields, ',')).'"';

$desc = $_SESSION['sfm_f314_desc'];

//echo print_r($desc['aa aa']);
$temp_desc = $desc;
$desc = array();

$params = $fields;
$place_holders = implode(',', array_fill(0, count($params), '?'));
$params_type = implode('', array_fill(0, count($params), 's'));

$sql = 'SHOW COLUMNS FROM `'.$table.'` WHERE Field IN('.$place_holders.')';

@$result = $dbHandler->query($sql, 'ASSOC', $params, $params_type);
...
if($autodetect)
{
	$params = $fields;
	$place_holders = implode(',', array_fill(0, count($params), '?'));
	$params_type = implode('', array_fill(0, count($params), 's')) . 's';
	$params[count($params)] = $table;
	
	$sql = 'select CONSTRAINT_NAME,TABLE_NAME,COLUMN_NAME,REFERENCED_TABLE_NAME,REFERENCED_COLUMN_NAME from information_schema.key_column_usage where referenced_table_name is not null and COLUMN_NAME IN('.$place_holders.') and TABLE_NAME = ?';
 ...
 foreach($desc as $k => $v)
{
	if(empty($desc[$k]['validation']['special_char']) || $desc[$column]['validation']['special_char'] === '') 
		$desc[$k]['validation']['special_char'] = "',!,@,#,$,%,^,&,*,(,),|";
}
$_SESSION['sfm_f314_desc'] = $desc;
 ...
 if(!empty($continue))
 {
    ...
    foreach($desc as $key=>$val)
    {
     ...
     if(!isset ($_POST['lbl_'.str_replace(' ', '_', $key)]) || trim($_POST['lbl_'.str_replace(' ', '_', $key)]) == '')
     {
       ...
       $desc[$key]['Label'] = clean_input($_POST['lbl_'.  str_replace(' ', '_', $key)]);
    ...
    if($valid)
    {
        //$_SESSION["sfm_f314_labels"] = $labels;
        header("location: step_5.php");
    }
 }

 // foreach($desc as $key=>$val) var_dump($val);
?>
<html>
...
<body style="overflow-x:hidden;">
 ...
 <div id="pop_layout" style="display: none; position: absolute; width: 100%; height: 100%; margin: -9px; text-align: center;z-index: 2;">
	...
	<tr>
		<td align="center" width="64" style="background-repeat: y" valign="top" background="images/leftadd.jpg">
			...
			<table border="0" width="100%" id="table8" height="331">
				<tr>
					<td height="18" colspan="2" class="step_title">Settings <a href="" onMouseOver="stm(Step_4[0],Style);" onClick="return false;" onMouseOut="htm()"> <img src="images/Help.png" border="0"></a>
					<button id="exit" style="position: relative;left: 410px;font-size: 11px;cursor: pointer;" onclick="return false;">Disconnect &amp; Exit</button></td>
				</tr>
				<tr>
		...
		<table cellspacing="0" cellpadding="0" width="100%" border="0" align="center" id="table9">
			<tr>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    <select style="width:110px;" name="txt_field_<?php echo str_replace(' ', '_', $key); ?>">
                                                        ...
                                                        <?php
                                                        $columns = $dbHandler->query('SHOW COLUMNS FROM `'.$val['REFERENCED_TABLE_NAME'].'`', 'ASSOC');
				...
				<tr>
					...
					<a href='step_3.php'><img src="images/03.jpg" border=0 width="170" height="34"></a></td>
					<td align="center">
