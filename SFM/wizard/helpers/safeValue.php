...
function clean_input($str, $allowSingleQoute = false)
{
	...
	foreach($specials as $key => $value) $str = str_replace($value, '', $str);
	$str = trim($str);
	return $str;
}
...
function clean_array($arr, $allowSingleQoute = false)
{
	...
	{
		if(is_array($value)) $cleaned_array[clean_input($key, $allowSingleQoute)] = clean_array($value, $allowSingleQoute);
		else $cleaned_array[clean_input($key, $allowSingleQoute)] = clean_input($value, $allowSingleQoute);
	}
...
function is_clean($str, $no_space = false, $only_alphanumeric = true) {
	...
	if($no_space) $_no_space = 'true';
	else $_no_space = 'false';
	if($only_alphanumeric) $_only_alphanumeric = 'true';
	else $_only_alphanumeric = 'false';
