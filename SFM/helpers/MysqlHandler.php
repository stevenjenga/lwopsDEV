...
class MysqlHandler {
    ...
    private function connect() {
        ...
        if (!$connection) {
            $this->debug_mode('Mysql::connect', 'error', 'Connection failed: ' . mysql_error());
            return false;
        }
        $this->debug_mode('Mysql::connect', 'success', 'Connected successfully');
        return $connection;
    }
    ...
    public function select_database($db) {
        ...
        if (!mysql_select_db($this->db)) {
            $this->debug_mode('Mysql::select_database', 'error', 'Can\'t select database: ' . mysql_error());
            return false;
        }
        $this->debug_mode('Mysql::select_database', 'success', 'Select database successfully');
        $this->command('SET FOREIGN_KEY_CHECKS = 0;');
        return true;
    }
    ...
    public function query($sqlStatement, $keyType = "NUM", $params = array()) { // $keyType = MYSQL_ASSOC, MYSQL_NUM, MYSQL_BOTH
        $this->debug_mode('Mysql::query', 'info', $sqlStatement);
        ...
        if ($this->link) {
            ...
            if (count($params) > 0) {
                $this->debug_mode('Mysql::query', 'info', 'Query parameters: ' . implode($params, ', '));
                $result = @mysql_query($this->bind_params($sqlStatement, $params));
            } else
                $result = $result = @mysql_query($sqlStatement);

            if (!$result) {
                $this->debug_mode('Mysql::query', 'error', 'Query failed ' . mysql_error());
                return false;
            } else {
                $this->numOfRows = @mysql_num_rows($result);
                $this->debug_mode('Mysql::query', 'success', 'Query success: it returns ' . $this->numOfRows . ' rows!');
    ...
    private function get_result($result, $keyType) {
        $fetchedData = array();
        while ($row = @mysql_fetch_array($result, $keyType))
            $fetchedData[] = $row;
        return $fetchedData;
    }

    public function command($sqlStatement, $params = array()) {
        $this->debug_mode('Mysql::command', 'info', $sqlStatement);
        ...
        if ($this->link) {
            ...
            if (count($params) > 0) {
                $this->debug_mode('Mysql::command', 'info', 'Command parameters: ' . implode($params, ', '));
                $result = @mysql_query($this->bind_params($sqlStatement, $params));
            } else
                $result = @mysql_query($sqlStatement); // query SQL Statement
            if ($result) {
                $this->numOfRows = mysql_affected_rows();
                $this->debug_mode('Mysql::command', 'success', "command success: it affects " . $this->numOfRows . " rows");
                return true;
            } else {
                $this->debug_mode('Mysql::command', 'error', 'Command failed ' . mysql_error());
                return false;
            }
        }
    }
