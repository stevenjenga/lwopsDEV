  var obj_btn;
    function open_popup(obj)
    {
        ...
        var str = '';
        if(($(obj_btn).attr('data_type').toString().indexOf('varchar') != -1  || $(obj_btn).attr('data_type').toString().indexOf('text') != -1 || $(obj_btn).attr('data_type').toString().indexOf('char') != -1))
        {
            str += '<tr>';
            ...
            str = '<tr>';
            str += '<td style="font-size: 10px;">Prevent Special Characters:</td>';
        ...
        else if(($(obj_btn).attr('data_type').toString().indexOf('int') != -1 || $(obj_btn).attr('data_type').toString().indexOf('float') != -1) && $(obj_btn).attr('data_type').toString().indexOf('tinyint') == -1)
        {
         ...
         else if($(obj_btn).attr('data_type').toString().indexOf('datetime') != -1 || $(obj_btn).attr('data_type').toString().indexOf('date') != -1)
        {
           str += '<tr>';
    ...
    $(function(){
       ...
       $('#select_regx').live('change',function(){
        ...
        $('#select_table').change(function(){
                ...
                $.post('details_form_step_ajax.php', {'column':column,'get_columns':table}, function(data){
                    //alert(data);
                    data = eval(data);
        ...
        $('.relation').click(function(){
           ...
           $('#btnSave_Rel').click(function(e){
               ...
               {
                   var column = $(obj_btn).attr('column');
                   $.post('details_form_step_ajax.php', {'column':column,'remove_relation':'true'},function(){
                       close_popup();
                   });
               ...
               else
               {
                    ...
                    $.post('details_form_step_ajax.php', {'column':column,'TextField':TextField,'REFERENCED_COLUMN_NAME':REFERENCED_COLUMN_NAME,'REFERENCED_TABLE_NAME':REFERENCED_TABLE_NAME}, function(data){
                    if(data == 'true')
                    {
...
//                
            ...
            if(valid)
            {
               $.post('details_form_step_ajax.php', {'column':column,'msg':msg,'regx':regx,'special_char':special_char,'from':from,'to':to,'regx_type':regx_type}, function(data){
                    if(data == 'true')
                    {
