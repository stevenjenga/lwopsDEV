	...
	class DatabaseHandler
	{
		...
		{       
			...
			if(extension_loaded('mysqli') && function_exists('mysqli_stmt_get_result') && ($this->extension === '' || $this->extension === 'mysqli')) 
			{
				...
				$selected_db_ok = true;
				if($db !== '' && $this->link->link !== false) $selected_db_ok = $this->select_database($db);
				if($this->link->link === false || $selected_db_ok === false) $this->link = false;
			}
		...
		{
			if($this->link === false || $this->link->link === false) return true;
			else return false;
		}
